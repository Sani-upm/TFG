...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  7 15:29:07 2023
...
globs = glob.iglob('MERCADONA_BANCO_SI/**.pdf')
...
for a in globs:
    ...
    if cuatro is not None:
       if flag==0: 
           dict={'FECHA':factura[3],'BANCO':'SI','TIPO':tipo,'FACTURA':factura[1],'PROVEEDOR':proveedor,'CIF':cif,'REFERENCIA':ref,'FLUJO':flujo,'BASE':-cuatro[1],'IVA':cuatro[2],'TIPO IVA':4,'INTRA': None,'TOTAL':-cuatro[3],'TOTAL FACT':-total_factura}
           data.append(dict)
           flag=1
       else:
           dict={'FECHA':factura[3],'BANCO':'SI','TIPO':tipo,'FACTURA':factura[1],'PROVEEDOR':proveedor,'CIF':cif,'REFERENCIA':ref,'FLUJO':flujo,'BASE':-cuatro[1],'IVA':cuatro[2],'TIPO IVA':4,'INTRA': None,'TOTAL':-cuatro[3],'TOTAL FACT':None}
           data.append(dict)
       
    if cinco is not None:
        if flag==0:
            dict={'FECHA':factura[3],'BANCO':'SI','TIPO':tipo,'FACTURA':factura[1],'PROVEEDOR':proveedor,'CIF':cif,'REFERENCIA':ref,'FLUJO':flujo,'BASE':-cinco[1],'IVA':cinco[2],'TIPO IVA':5,'INTRA': None,'TOTAL':-cinco[3], 'TOTAL FACT': -total_factura}
            data.append(dict)
            flag=1
        else:
            dict={'FECHA':factura[3],'BANCO':'SI','TIPO':tipo,'FACTURA':factura[1],'PROVEEDOR':proveedor,'CIF':cif,'REFERENCIA':ref,'FLUJO':flujo,'BASE':-cinco[1],'IVA':cinco[2],'TIPO IVA':5,'INTRA': None,'TOTAL':-cinco[3], 'TOTAL FACT': None}
            data.append(dict)
       
    if diez is not None:
        if flag==0:
            dict={'FECHA':factura[3],'BANCO':'SI','TIPO':tipo,'FACTURA':factura[1],'PROVEEDOR':proveedor,'CIF':cif,'REFERENCIA':ref,'FLUJO':flujo,'BASE':-diez[1],'IVA':diez[2],'TIPO IVA':10,'INTRA': None,'TOTAL':-diez[3], 'TOTAL FACT': -total_factura}
            data.append(dict)
            flag=1
        else:
            dict={'FECHA':factura[3],'BANCO':'SI','TIPO':tipo,'FACTURA':factura[1],'PROVEEDOR':proveedor,'CIF':cif,'REFERENCIA':ref,'FLUJO':flujo,'BASE':-diez[1],'IVA':diez[2],'TIPO IVA':10,'INTRA': None,'TOTAL':-diez[3], 'TOTAL FACT': None}
            data.append(dict)
            
    if veinte is not None:
        if flag==0:
            dict={'FECHA':factura[3],'BANCO':'SI','TIPO':tipo,'FACTURA':factura[1],'PROVEEDOR':proveedor,'CIF':cif,'REFERENCIA':ref,'FLUJO':flujo,'BASE':-veinte[1],'IVA':veinte[2],'TIPO IVA':21,'INTRA': None,'TOTAL':-veinte[3], 'TOTAL FACT':-total_factura}
            data.append(dict)
            flag=1
        else:
            dict={'FECHA':factura[3],'BANCO':'SI','TIPO':tipo,'FACTURA':factura[1],'PROVEEDOR':proveedor,'CIF':cif,'REFERENCIA':ref,'FLUJO':flujo,'BASE':-veinte[1],'IVA':veinte[2],'TIPO IVA':21,'INTRA': None,'TOTAL':-veinte[3], 'TOTAL FACT': None}
            data.append(dict)
